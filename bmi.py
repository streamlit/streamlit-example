...
def bmi_advice(test_results):
    ...
    weightloss = False
    if bmi >=27.5: 
        output_string = "You are obese. BMI is " + str(bmi)
        weightloss = True
    elif bmi >= 23:
        output_string = "You are overweight. BMI is " + str(bmi)
        weightloss = True
    elif bmi < 18.5:
        output_string = "You are underweight. BMI is " + str(bmi)+ " Consider increasing food intake, for example, by taking smaller, frequent healthy meals. Increase protein intake by taking more lean meats, fish, eggs, dairy, legumes and nuts. Do strength training to build up muscles."
    else:
        output_string = "You have a healthy BMI. BMI is " + str(bmi)
    if weightloss:
