...
# Create a folder for image storage
...
def render_predict():
    ...
    # st.write(powerbi_embed_code, unsafe_allow_html=True)
    ...
    if uploaded_image is not None:
        ...
        # prediction_rating = st.slider("Rate the prediction (1-5)", min_value=1, max_value=5, step=1, value=3)
        # st.write(f"You rated the prediction as: {prediction_rating}")
        ...
        # for i in range(1, 6):
        #     star_icon = "⭐" * i
        #     if stars.button(star_icon, key=f"star_{i}"):
        #         rating.markdown(f"You rated the prediction as {i} star(s).")



# Create a function to render the "Dashboard" section
def render_dashboard():
    st.title("Dashboard Section")
    # Add your code for the dashboard section here
...
def render_feedback():
    ...
    # Create star rating buttons for feedback
    rating = st.empty()
    stars = st.empty()
    for i in range(1, 6):
        star_icon = "⭐" * i
        if stars.button(star_icon, key=f"feedback_star_{i}"):
            rating.markdown(f"You rated the feedback as {i} star(s).")
...
def main():
    ...
    navigation_option = st.sidebar.selectbox("Go to", ["Predict", "Dashboard", "Feedback"])

    # Display the selected section based on the selectbox choice
    if navigation_option == "Predict":
        render_predict()
    elif navigation_option == "Dashboard":
        render_dashboard()
