...
with tab1:
    st.altair_chart(alt.Chart(pd.DataFrame(source2)).mark_bar().encode(x='x', y='pls'), theme="streamlit", use_container_width=True)
with tab2:
    st.altair_chart(alt.Chart(pd.DataFrame(source1)).mark_bar().encode(x='x', y='pls'), theme="streamlit", use_container_width=True)
