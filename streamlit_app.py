...
def main():
    ...
    if uploaded_file is not None:
        ...
        df_cluster1 = df[['История', 'Психология', 'Математика и/или физика', 'Интернет и социальные сети']]
        ...
        df_cluster4 = df[
            ['Престижность места учёбы (школы, колледжа, университета)', 'Учиться в другом российском городе',
             'Учиться за рубежом',
             'Учёба на «отлично»', 'Изучить дополнительный иностранный язык', 'Изучить языки программирования',
        ...
        df_cluster5 = df[['Мой город комфортный для проживания', 'Мой город безопасный',
        'Мой город экологически чистый',
        'В моём городе живут самые красивые люди','Мне легко налаживать контакт с жителями нашего города']]
        ...
        df_cluster8 = df[['Много зарабатывать', 'Работать не по специальности', 'Заниматься бизнесом',
                          ...
                          'Приносить пользу обществу', 'Работать в СМИ']]
        ...
        cluster2_pairs = [('Химия', True), ('Экономика и менеджмент', True), ('Биология', True),
                          ('Иностранные языки', True), ('Медицина', True),
        ...
        cluster4_pairs = [('Престижность места учёбы (школы, колледжа, университета)', True),
                          ('Учиться в другом российском городе', False),
                          ('Учиться за рубежом', False), ('Учёба на «отлично»', True),
                          ('Изучить дополнительный иностранный язык', True), ('Изучить языки программирования', True),
                          ('Представлять школу/колледж/университет на конкурсах, спортивных соревнованиях, олимпиадах',
                           True)]
        # Создание массивов пар для df_cluster5
        cluster5_pairs = [('Мой город комфортный для проживания', True),
        ...
        cluster6_pairs = [('Я бы хотел работать в своём городе', True),
        ...
        cluster7_pairs = [('В моём городе много исторических достопримечательностей', True),
                          ('Мой город – это высокоразвитая культура его жителей', True),
                          ('В моём городе большие пробки', False),
                          ('В моём городе достаточно культурных центров, музеев, театров, кинотеатров', True),
                          ('Я с гордостью осознаю, что живу в своём городе', True)]
        cluster8_pairs = [('Много зарабатывать', True), ('Работать не по специальности', True),
                          ('Заниматься бизнесом', True),
                          ('Работать на государственной службе', True), ('Заниматься волонтерством', True),
                          ('Аспирантура и учёная степень', True), ('Приносить пользу обществу', True),
                          ('Работать в СМИ', True)]
        cluster9_pairs = [('У меня в городе есть что посмотреть даже бывалым туристам и путешественникам', True),
                          ('Я периодически устаю от своего города и хочется сменить обстановку', False),
                          ('Я не рассматриваю вариантов переехать из моего города', True)]

        all_clusters_pairs = [cluster1_pairs, cluster2_pairs, cluster3_pairs, cluster4_pairs, cluster5_pairs,
                              cluster6_pairs, cluster7_pairs,cluster8_pairs,cluster9_pairs]
        if st.button('Удалить выступающие усы'):
            for clusters_pairs in all_clusters_pairs:
                ...
                # Create a copy of the data before removing outliers
                data_copy = data.copy()
        ...
        if st.button('Показать важность фичей:'):
            ...
            df['target'] = df[questions].apply(
                lambda x: 1 if x[0] > 3 and x[1] > 3 or x[0] < 3 and x[1] < 3 else 0, axis=1)
            ...
            for i, X in enumerate(clusters):
                ...
                question_labels = ['Переезд возможен', 'Не рассматриваю переезд']  # Подписи к вопросам
                ...
                ax.set_title('Важность фичей в предсказании о переезде', fontsize=12)  # Увеличение размера шрифта

                # Добавление ярлыков на график
                for i, v in enumerate(x_values):
                    ax.text(v, i, str(round(v, 2)), color='black', va='center',
                            fontsize=12)  # Увеличение размера шрифта

                # Добавление подписей к цветам
                legend_elements = [Patch(facecolor='blue', edgecolor='black', label='Переезд возможен'),
